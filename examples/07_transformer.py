...
class Attention(nnx.Module):
  ...
  def __init__(self, cfg: Config, *, ctx: nnx.Context):
    ...
    self.WQ = nnx.Param(
        dense_init(key, (cfg.embed, cfg.heads, cfg.depth), cfg.param_dtype, 0, (1, 2)),
    ...
    key = ctx.make_rng("params")
    self.WO = nnx.Param(
        dense_init(key, (cfg.heads, cfg.depth, cfg.embed), cfg.param_dtype, (0, 1), 2),
  ...
  def __call__(self, cfg: Config, x_q, x_kv, mask=None, *, ctx: nnx.Context):
    q = jnp.einsum("bse,enh->bsnh", x_q, self.WQ.astype(cfg.dtype)).astype(jnp.float32)
    k = jnp.einsum("bte,enh->btnh", x_kv, self.WK.astype(cfg.dtype)).astype(jnp.float32)
    v = jnp.einsum("bte,enh->btnh", x_kv, self.WV.astype(cfg.dtype))

    index = None
    if cfg.decode:
      index = self.index
      one_hot_indices = jax.nn.one_hot(self.index, cfg.max_length, dtype=cfg.dtype)
      ...
      mask = jnp.logical_and(cache_mask if mask is None else mask, cache_mask).astype(
          cfg.dtype
      )
...
class DecoderBlock(nnx.Module):
  ...
  def __init__(self, cfg: Config, *, ctx: nnx.Context):
    ...
    self.scale1 = nnx.Param(jnp.ones((cfg.embed,), cfg.param_dtype), P(sharding.embed))
    self.scale2 = nnx.Param(jnp.ones((cfg.embed,), cfg.param_dtype), P(sharding.embed))
...
class Decoder(nnx.Module):
  ...
  def __init__(self, cfg: Config, *, ctx: nnx.Context):
    ...
    self.unembed = nnx.Param(
        dense_init(ctx.make_rng("params"), (cfg.embed, cfg.vocab), jnp.float32, 0, 1),
    ...
    if cfg.scanned:
      ...
      self.layers = nnx.Sequence(DecoderBlock(cfg, ctx=ctx) for _ in range(cfg.layers))
