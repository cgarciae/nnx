...
def scan_apply(
  ...
  if len(lengths) > 1:
    raise ValueError(
        f"Inconsistent lengths between variable_axes states and " f"axes_arg: {lengths}"
    )
  elif len(lengths) == 0:
    if options.length is None:
      raise ValueError(
  ...
  module.update_state((*axes_states, carry_state, broadcast_state_new, carry_state_new))
...
class Remat(Module, tp.Generic[M], metaclass=RematMeta):
  ...
  def __init__(
    ...
    self.module_constructor = module_constructor
    self.remat_module = self.module_constructor(*module_init_args, **module_init_kwargs)
