...
class Linear(Module):
  ...
  def __call__(self, inputs: Array) -> Array:
    ...
    inputs, kernel, bias = dtypes.promote_dtype(inputs, kernel, bias, dtype=self.dtype)
    y = self.dot_general(
        inputs,
        kernel,
        (((inputs.ndim - 1,), (0,)), ((), ())),
...
class Embed(Module):
  ...
  def __init__(
      ...
      *,
      dtype: tp.Optional[Dtype] = None,
      param_dtype: Dtype = jnp.float32,
      embedding_init: tp.Callable[[PRNGKey, Shape, Dtype], Array] = default_embed_init,
      ctx: contextlib.Context,
  ):
    self.embedding = nnx.Param(
        embedding_init(ctx.make_rng("params"), (num_embeddings, features), param_dtype)
    )
  ...
  def __call__(self, inputs: Array) -> Array:
    ...
    # work correctly.
    (embedding,) = dtypes.promote_dtype(self.embedding, dtype=self.dtype, inexact=False)
