...
class Pytree(reprlib.Representable, metaclass=PytreeMeta):
  ...
  def _setattr(self: P, name: str, value: tp.Any):
    ...
    elif "_pytree__is_mutable" not in vars_dict and not self._pytree__class_is_mutable:
      raise AttributeError(f"{type(self)} is immutable, trying to update field {name}")
